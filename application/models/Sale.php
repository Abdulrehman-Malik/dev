...
class Sale extends CI_Model
{
	...
	public function create_temp_table_gl(array $inputs)
	{
		...
		if(empty($inputs['sale_id']))
		{
			if(empty($this->config->item('date_or_time_format')))
			{
				$where = 'DATE(sl.ledger_time) BETWEEN ' . $this->db->escape($inputs['start_date']) . ' AND ' . $this->db->escape($inputs['end_date']);
			}
			else
			{
				$where = 'sl.ledger_time BETWEEN ' . $this->db->escape(rawurldecode($inputs['start_date'])) . ' AND ' . $this->db->escape(rawurldecode($inputs['end_date']));
			}
		}
		else
		{
			$where = 'sl.ledger_id = ' . $this->db->escape($inputs['ledger_id']);
		}

		if(!empty($inputs['supplier_id']))
		$where = $where. ' and  sl.supplier_id = ' . $inputs['supplier_id'];
		...
		// create a temporary table to contain all the sum of taxes per sale item
		$this->db->query('CREATE  temporary TABLE IF NOT EXISTS ' . $this->db->dbprefix('temp_gl') .
			' (INDEX(ledger_id), INDEX(supplier_id)) ENGINE=MEMORY
			(
				...
				SELECT sl.ledger_id,
					...
					sl.supplier_id,
					(ROUND(sl.debit_amount, ' . $decimals . ')) AS debit_amount,
					(ROUND(sl.credit_amount, ' . $decimals . ')) AS credit_amount,
					(ROUND(sl.balance, ' . $decimals . ')) AS balance,
					ledger_time,
					reference,
					trans_type,
					ep.username as user,
					sl.employee_id
				FROM ' . $this->db->dbprefix('supplier_ledgers') . ' AS sl
				LEFT OUTER JOIN ' . $this->db->dbprefix('suppliers') . ' AS sp
					ON sl.supplier_id = sp.person_id
				LEFT OUTER JOIN ' . $this->db->dbprefix('employees') . ' AS ep
		...
		$this->db->query('insert into ospos_temp_gl select *from ospos_temp_gl_op;');

		
		
	}

	public function create_temp_ledger_op_balance(array $inputs)
	{
		...
		if(empty($inputs['ledger_id']))
		{
			if(empty($this->config->item('date_or_time_format')))
			{
				$where = 'DATE(sl.ledger_time) < ' . $this->db->escape($inputs['start_date']);
			}
			else
			{
				$where = 'sl.ledger_time < ' . $this->db->escape(rawurldecode($inputs['start_date'])) ;
			}
		}
		else
		{
		...
		// create a temporary table to contain all the sum of taxes per sale item
		$this->db->query('CREATE Temporary  TABLE IF NOT EXISTS ' . $this->db->dbprefix('temp_gl_op') .
			' (INDEX(supplier_id)) ENGINE=MEMORY
			(
