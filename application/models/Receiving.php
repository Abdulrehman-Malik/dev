...
class Receiving extends CI_Model
{
	...
	public function save($items, $supplier_id, $employee_id, $comment, $reference, $payment_type, $receiving_id = FALSE,$amount_tendered=0,$mode)
	{
		...
		foreach($items as $line=>$item)
		{
			...
			$total_amt = $total_amt + ($item['price'] * $items_received);
			//echo $item['price'] * $item['receiving_quantity'] ;
		...
		$gl_data = array(
			...
			'supplier_id' => $this->Supplier->exists($supplier_id) ? $supplier_id : NULL,
			'employee_id' => $employee_id,
			'reference' => 'Rec-'.$receiving_id,
			'debit_amount' => 0,
			'credit_amount' => $total_amt,
			'payment_type' => "Stock Receiving"
		);
