...
class Payment_lib
{
	...
	{
		if(!$this->CI->session->userdata('pv_cart'))
		{
			$this->set_cart(array());
		}

		return $this->CI->session->userdata('pv_cart');
	}

	public function set_cart($cart_data)
	{
		$this->CI->session->set_userdata('pv_cart', $cart_data);
	}

	public function empty_cart()
	{
		$this->CI->session->unset_userdata('pv_cart');
	}

	public function get_supplier()
	{
		if(!$this->CI->session->userdata('pv_supplier'))
		{
			$this->set_supplier(-1);
		}

		return $this->CI->session->userdata('pv_supplier');
	}

	public function set_supplier($supplier_id)
	{
		$this->CI->session->set_userdata('pv_supplier', $supplier_id);
	}

	public function remove_supplier()
	{
		$this->CI->session->unset_userdata('pv_supplier');
	}

	public function get_mode()
	{
		if(!$this->CI->session->userdata('pv_mode'))
		{
			$this->set_mode('receive');
		}

		return $this->CI->session->userdata('pv_mode');
	}

	public function set_mode($mode)
	{
		$this->CI->session->set_userdata('pv_mode', $mode);
	}
	
	public function clear_mode()
	{
		$this->CI->session->unset_userdata('pv_mode');
	}

	public function get_stock_source()
	{
		if(!$this->CI->session->userdata('pv_stock_source'))
		{
			$this->set_stock_source($this->CI->Stock_location->get_default_location_id('receivings'));
		}

		return $this->CI->session->userdata('pv_stock_source');
	}
	
	public function get_comment()
	{
		// avoid returning a NULL that results in a 0 in the comment if nothing is set/available
		$comment = $this->CI->session->userdata('pv_comment');
	...
	{
		$this->CI->session->set_userdata('pv_comment', $comment);
	}
	
	public function clear_comment()
	{
		$this->CI->session->unset_userdata('pv_comment');
	}
   
	public function get_reference()
	{
		return $this->CI->session->userdata('pv_reference');
	}
	
	public function set_reference($reference)
	{
		$this->CI->session->set_userdata('pv_reference', $reference);
	}
	
	public function clear_reference()
	{
		$this->CI->session->unset_userdata('pv_reference');
	}
	
	public function get_total()
	{
		$total = 0;
		$total  = $this->CI->session->userdata('pv_total');
		 return $total;
	}
	
	public function set_total($amt)
	{
		$this->CI->session->set_userdata('pv_total', $amt);
	}
	public function set_balance($amt)
	{
		$this->CI->session->set_userdata('pv_balance', $amt);
	}
	public function get_balance()
	{
		return $this->CI->session->userdata('pv_balance');
	}
	
	public function clear_total()
	{
		//$this->CI->session->unset_userdata('pv_total');
	}
	

	public function is_print_after_sale()
	{
		return $this->CI->session->userdata('pv_print_after_sale') == 'true' ||
				$this->CI->session->userdata('pv_print_after_sale') == '1';
	}
	
	public function set_print_after_sale($print_after_sale)
	{
		return $this->CI->session->set_userdata('pv_print_after_sale', $print_after_sale);
	}
	
	public function set_stock_source($stock_source)
	{
		$this->CI->session->set_userdata('pv_stock_source', $stock_source);
	}
	
	public function clear_stock_source()
	{
		$this->CI->session->unset_userdata('pv_stock_source');
	}
	
	public function get_stock_destination()
	{
		if(!$this->CI->session->userdata('pv_stock_destination'))
		{
			$this->set_stock_destination($this->CI->Stock_location->get_default_location_id('receivings'));
		}

		return $this->CI->session->userdata('pv_stock_destination');
	}

	public function set_stock_destination($stock_destination)
	{
		$this->CI->session->set_userdata('pv_stock_destination', $stock_destination);
	}
	
	public function clear_stock_destination()
	{
		$this->CI->session->unset_userdata('pv_stock_destination');
	}
