...
class Reports extends Secure_Controller
{
	...
	//Summary Ledger Items report
	public function summary_ledgers($start_date, $end_date, $sale_type, $location_id = 'all')
	{
		$inputs = array('start_date' => $start_date, 'end_date' => $end_date, 'sale_type' => $sale_type, 'location_id' => $location_id);

		$this->load->model('reports/Summary_ledgers');
		$model = $this->Summary_ledgers;
		...
		$data = array(
			'title' => $this->lang->line('reports_ledgers_summary_report'),
			'subtitle' => $this->_get_subtitle_report(array('start_date' => $start_date, 'end_date' => $end_date)),
			'headers' => $this->xss_clean($model->getDataColumns()),
			'data' => $tabular_data,
			'summary_data' => $summary
	...
	public function specific_suppliersgls_input()
	{
	...
	public function specific_suppliersgls($start_date, $end_date, $supplier_id, $sale_type)
	{
		$inputs = array('start_date' => $start_date, 'end_date' => $end_date, 'supplier_id' => $supplier_id, 'sale_type' => $sale_type);

		$this->load->model('reports/Specific_suppliersgls');
		$model = $this->Specific_suppliersgls;
		...
		foreach($report_data as $row)
		{
			...
			$tabular_data[] = $this->xss_clean(array(
				...
				'ledger_time' => to_datetime(strtotime($row['ledger_time'])),
				'trans_type' => $row['trans_type'],
				'payment_type' => $row['payment_type'],
	...
	{
		//echo $this->lang->line('reports_complete');
		//return;
	...
	public function inventory_summary($location_id = 'all', $item_count = 'all',$specific_input_data = 'all')
	{
		//echo $specific_input_data;
		$inputs = array('location_id' => $location_id, 'item_count' => $item_count,'specific_input_data'=>$specific_input_data);
